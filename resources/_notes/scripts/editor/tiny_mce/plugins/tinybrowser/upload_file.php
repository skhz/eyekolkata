...
if(session_id() != '')
	{
	if(!isset($_SESSION[$tinybrowser['sessioncheck']])) { echo 'Error! 1'; exit; }
	}
	
// Check hash is correct (workaround for Flash session bug, to stop external form posting)
//echo $_GET['obfuscate']."<br />".md5($_SERVER['DOCUMENT_ROOT'].$tinybrowser['obfuscate']);
...
if ($_FILES['Filedata']['tmp_name'] && $_FILES['Filedata']['name'])
	...
	//echo $file_name;
	//echo $tinybrowser['link']['image']."<br />";
	//$destination = 'http://www.cwimports.com.au/newsite/images/tinyImage';
	...
	if(is_dir($tinybrowser['docroot'].$dest_folder))
		{
			...
			$success = move_uploaded_file($_FILES['Filedata']['tmp_name'],$tinybrowser['docroot'].$dest_folder.'/newsite/images/tinyImage/'.$file_name);
		}
	if($success)
		{
